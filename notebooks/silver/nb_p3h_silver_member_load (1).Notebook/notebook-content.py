...
def silver_member_ingestion_with_logging(table_id, pipeline_run_id, pipeline_name):
    start_time = datetime.now()

    try:
        target_table = f"{silver_schema}.member"
        business_key = "member_id"
        surrogate_key = "member_sk"

        df = prepare_silver_member()
        ...
        if spark.table(target_table).count() > 0:
            ...
            df_for_dq = (
                ...
                .filter(
                    (F.col("hash_col_right").isNull()) |  # New member
                    (F.col("hash_col") != F.col("hash_col_right"))  # Changed member
                )
                .drop("hash_col_right")
            )
        else:
...
silver_member_ingestion_with_logging(table_id, pipeline_run_id, pipeline_name)
